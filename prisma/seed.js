...
const userId = process.env.seed_movie_creator;
...
//Create a function to seed the database. When the script is run our dB will have 10 movies.

const movieSeeder = async () => {
  for (const movie of movies) {
    await prisma.movie.create({ data: movie });
    console.log(`${movie.title} iko nditni`);
  }
};

//Now we call the seeding function
movieSeeder().catch((e)=>{
  console.error(e)
  process.exit(1) //Kill the process if errors
}).finally(async()=>{
  await prisma.$disconnect() //We disconnect from the db after seeding is done.
})
